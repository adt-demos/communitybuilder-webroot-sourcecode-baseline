...
class RegistryEditView {
	...
	protected function _renderLine( $param, $result, $control_name='params', $htmlFormatting = 'div', $htmlid = true, /** @noinspection PhpUnusedParameterInspection */ $view = false ) {
		...
		if ( $htmlFormatting == 'table' ) {
			$html[]			=	'<tr' . $htid . ' class="cbft_' . $type . ( $tag ? ' cbtt_' . $tag : null ) . ( $classes ? ' ' . htmlspecialchars( $classes ) : null ) . ( $conditioned ? ' hidden cbDisplayDisabled' : null ) . '">';
			...
			$html[]			=	'<div' . $htid . ' class="cbft_' . $type . ( $tag ? ' cbtt_' . $tag : null ) . ' form-group row no-gutters cb_form_line' . ( $twoLine ? ' cbtwolinesfield' : null ) . ( $classes ? ' ' . htmlspecialchars( $classes ) : null ) . ( $conditioned ? ' hidden cbDisplayDisabled' : null ) . '">';
			...
			if ( substr( $result[0], -2 ) == "%s" ) {
				...
				$html[]		=	'<span' . $htid . ' class="cbft_' . $type . ( $tag ? ' cbtt_' . $tag : null ) . ( $classes ? ' ' . htmlspecialchars( $classes ) : null ) . ( $conditioned ? ' hidden cbDisplayDisabled' : null ) . '">';
	...
	*/
	...
	private function checkParamCondition( $param )
	{
		...
		if ( $showInside && $param->getElementByPath( 'and' ) ) {
