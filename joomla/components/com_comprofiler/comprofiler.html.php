...
******************************/
	...
	static function userEdit( $user, $option = '', $submitvalue = '', $regErrorMSG = null ): void
	{
		...
		if ( $userEditTemplate ) {
			...
			$menuParams				=	Application::Cms()->getActiveMenuWithParams();

			$userEditLayout			=	( $menuParams->get( 'params/profile_edit_layout', '' ) ?: $userEditTemplate->params->getString( 'profile_edit_layout', $userEditLayout ) );
			$userEditCustom			=	( $menuParams->get( 'params/profile_edit_layout_custom', '' ) ?: $userEditTemplate->params->getRaw( 'profile_edit_layout_custom', $userEditCustom ) );
	...
	static function userProfile( $user, $option = '', $submitvalue = '' ): void
	{
		...
		if ( $template ) {
			...
			if ( ( $user->getInt( 'id', 0 ) === Application::MyUser()->getUserId() ) && ( Application::Input()->getString( 'get/viewas', '' ) !== 'other' ) ) {
				$userViewLayout	=	( $menuParams->get( 'params/profile_view_layout_home', '' ) ?: $template->params->getString( 'profile_view_layout_home', $userViewLayout ) );
				$userViewCustom	=	( $menuParams->get( 'params/profile_view_layout_home_custom', '' ) ?: $template->params->getRaw( 'profile_view_layout_home_custom', $userViewCustom ) );
				...
				$userViewLayout	=	( $menuParams->get( 'params/profile_view_layout_other', '' ) ?: $template->params->getString( 'profile_view_layout_other', $userViewLayout ) );
				$userViewCustom	=	( $menuParams->get( 'params/profile_view_layout_other_custom', '' ) ?: $template->params->getRaw( 'profile_view_layout_other_custom', $userViewCustom ) );
...
******************************/
	 ...
	 * @param int           $random
	 */
	static function usersList( &$row, $params, &$users, &$columns, &$fields, &$input, $search, $searchmode, $pageNav, &$myUser, &$searchableFields, &$searchValues, &$tabs, $errorMsg, $listAll = true, $random = 0 ) {
		...
		if ( ! $params->get( 'list_hide_metadata', false ) ) {
			$_CB_framework->setPageTitle( strip_tags( cbReplaceVars( $row->title, $myUser, false, false ) ) );
		...
		$return							=	'<div class="cbUsersList cb_template cb_template_' . selectTemplate( 'dir' ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : null ) . '">'
										.		'<form action="' . $_CB_framework->viewUrl( array( 'userslist', 'listid' => (int) $row->listid ), true, array( 'searchmode' => (int) $searchmode ) ) . '" method="post" id="adminForm" name="adminForm" class="d-flex flex-column gap-3 form-auto m-0 cb_form cbValidation">'
										...
										.			Application::Session()->getFormTokenInput()
										.			$_PLUGINS->callTemplate( $cbTemplate, 'List', 'drawListHead', array( &$input, $row->listid, $pageNav->total, $showAll, $searchTabContent, $isSearching, $baseUrl, $listTitleHtml, $listDescription, $criteriaTitle, $resultsTitle, $listAll, $listSelector, $isCollapsed, $searchmode, $params->getBool( 'list_hide_header', false ) ), 'html' );
		...
		if ( ( $searchmode == 0 ) || ( ( $searchmode == 1 ) && $searchValuesCount ) || ( $searchmode == 2 ) ) {
			...
			if ( $canPage ) {
				$return					.=			'<div class="cbUserListPagination cbUserListPaginationBottom">'
...
******************************/
	...
	static function lostPassForm( /** @noinspection PhpUnusedParameterInspection */ $option ) {
		...
		$return					.=			'<div class="cb_forgot_line form-group row no-gutters cb_form_line" id="lostpassemail">'
								...
								.				'<div class="cb_field col-sm-9">'
								.					'<input type="email" name="checkemail" id="checkemail" class="form-control" size="30" maxlength="255"' . $emailValidation . ' />'
								.				'</div>'
								.			'</div>';
	...
	static function registerForm( $option, $emailpass, $user, $postvars = [], $regErrorMSG = null, $stillDisplayLoginModule = false, $regErrorLevel = 'error', bool $isInline = false, string $customLayout = '', ?bool $forceHTTPS = null ): void
	{
		...
		if ( $template ) {
			...
			$userRegLayout			=	( $menuParams->get( 'params/registration_layout', '' ) ?: $template->params->getString( 'registration_layout', $userRegLayout ) );
			$userRegCustom			=	( $menuParams->get( 'params/registration_layout_custom', '' ) ?: $template->params->getRaw( 'registration_layout_custom', $userRegCustom ) );
