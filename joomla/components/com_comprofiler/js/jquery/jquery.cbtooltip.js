				...
				cbtooltip.tooltip = cbtooltip.element.qtip({
					...
					events: {
						...
						show: function( event, api ) {
							...
							if ( tooltipModal ) {
								if ( api.options.position.adjust.scroll ) {
									$( 'body' ).addClass( 'cbTooltipModalOpen-' + api.id );
								}
							}
						...
						hidden: function( event, api ) {
							if ( tooltipModal && api.options.position.adjust.scroll ) {
								$( 'body' ).removeClass( 'cbTooltipModalOpen-' + api.id );
							}
						...
						focus: function( event, api ) {
							if ( tooltipModal ) {
								$( document ).on( 'keydown.cbtooltip.' + api.id, function( e ) {
									if ( e.which === 27 ) {
										api.hide();
									}
								});
						...
						blur: function( event, api ) {
							if ( tooltipModal ) {
								$( document ).off( 'keydown.cbtooltip.' + api.id );
							}
