(function($) {
		...
		init: function( options ) {
			return this.each( function () {
				...
				cbvalidate.validate = cbvalidate.element.validate({
					...
					ignore: '.cbValidationDisabled,.hidden,.disabled,.ignore,:disabled,[readonly],[type="hidden"]:not(.cbValidationAllowed),[contenteditable]:not(.cbValidationAllowed)',
					...
					onkeyup: function( element, event ) { // Validate on keyup
						// extends default keyup validation to check for validate-on to force keyup validation on first input
						...
						if ( element.name in this.submitted || element.name in this.invalid || ( $( element ).data( 'validate-on' ) === 'keyup' ) ) {
							this.element( element );
						}
	...
	function resizeImage( validator, file, previous, params ) {
		...
		// Always process AVIF as we need to convert it to PNG:
		...
		imgReader.onload = function( imgReaderEvent ) {
			...
			image.onload = function( imageEvent ) {
				...
				if ( ( ! alwaysResample ) && ( originalWidth === width ) && ( originalHeight === height ) && ( aspectRatio !== 3 ) ) {
					...
					$( '<input type="hidden"' + ( name.length ? ' name="' + name + '_image_data"' : '' ) + ' value="' + canvas.toDataURL( mimeType, 1 ) + '" class="cbValidationImageData" />' ).insertAfter( $( $that ) );
					...
					if ( ( aspectRatio === 3 ) && ( cropOrientation !== 'square' ) ) {
						$( `
							<div class="cbValidationImageDataCrop">
								...
								<div class="cbValidationImageDataCropArea">
									...
									<img src="${canvas.toDataURL( mimeType, 1 )}" class="cbValidationImageDataCropImage" />
								</div>
							</div>
						...
						const dragCrop = function( force ) {
							...
							if ( force === true ) {
								$( this ).siblings( '.cbValidationImageData' ).val( cropped.toDataURL( mimeType, 1 ) );
							} else {
								$( this ).closest( '.cbValidationImageDataCrop' ).siblings( '.cbValidationImageData' ).val( cropped.toDataURL( mimeType, 1 ) );
							}
						};
