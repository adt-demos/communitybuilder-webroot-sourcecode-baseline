...
class cbarticlesModel
{
	 ...
	 * @return null|string
	 */
	static public function getArticlesTotal( $where, $viewer, $user, $plugin )
	{
		global $_CB_database;
		...
		if ( $plugin->params->getBool( 'article_j_language', false ) && ( $viewer->getInt( 'id', 0 ) !== $user->getInt( 'id', 0 ) ) ) {
			$language		=	$viewer->getUserLanguage();

			if ( ! $language ) {
				$language	=	Application::Cms()->getLanguageTag();
			}

			$query			.=	"\n AND a." . $_CB_database->NameQuote( 'language' ) . " IN ( " . $_CB_database->Quote( $language ) . ", " . $_CB_database->Quote( '*' ) . ", " . $_CB_database->Quote( '' ) . " )";
		}
	 ...
	 * @return Table[]
	 */
	static public function getArticles( $paging, $where, $orderBy, $viewer, $user, $plugin )
	{
		global $_CB_database;
