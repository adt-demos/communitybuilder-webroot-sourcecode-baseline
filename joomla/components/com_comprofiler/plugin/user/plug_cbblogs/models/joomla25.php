...
class cbblogsBlogTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		$article		=	( checkJversion( '6.0+' ) ? new Content( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'content' ) );
	...
	public function store( $updateNulls = false )
	{
		...
		$new				=	( ! (int) $id );
		$table				=	( checkJversion( '6.0+' ) ? new Content( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'content' ) );
		$data				=	(array) $this;

		foreach ( $data as $k => $v ) {
			if ( in_array( $k, $this->getAllProperties(), true ) && ( $v !== null ) ) {
				continue;
			}
		...
		unset( $data['typeAlias'], $data['tagsHelper'] );

		if ( $new || ( ! $article->get( 'alias' ) ) || ( $article->get( 'title' ) !== ( $data['title'] ?? '' ) ) ) {
			$alias			=	Application::Router()->stringToAlias( ( $data['title'] ?? '' ) );

			while ( $table->load( [ 'alias' => $alias, 'catid' => ( $data['catid'] ?? 0 ) ] ) ) {
				$matches	=	null;
		...
		if ( $article->get( 'images' ) === null ) {
			$data['images']	=	'{"image_intro":"","image_intro_alt":"","float_intro":"","image_intro_caption":"","image_fulltext":"","image_fulltext_alt":"","float_fulltext":"","image_fulltext_caption":""}';
		}
		...
		if ( ! $new ) {
			$data['modified']		=	Application::Database()->getUtcDateTime();
			$data['modified_by']	=	(int) $user->get( 'id' );
			...
			if ( ( $article->get( 'created' ) === null ) || ( $article->get( 'created' ) === $article->getDbo()->getNullDate() ) ) {
				$data['created']	=	Application::Database()->getUtcDateTime();
			}

			if ( ( $article->get( 'publish_up' ) === null ) || ( $article->get( 'publish_up' ) === $article->getDbo()->getNullDate() ) ) {
				$data['publish_up']	=	Application::Database()->getUtcDateTime();
			}
...
class cbblogsCategoryTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		$category		=	( checkJversion( '6.0+' ) ? new Category( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'category' ) );
