...
abstract class cbforumsClass
{
	 ...
	 * @return stdClass
	 */
	public static function getModel(): stdClass
	{
		static $cache		=	null;
		...
		if ( ( ! class_exists( '\Kunena\Forum\Libraries\Forum\KunenaForum' ) ) || ( ! KunenaForum::enabled() ) ) {
			$cache			=	$model;
		...
		if ( ( ! $plugin ) || ( ! ( $plugin->absPath ?? '' ) ) ) {
			$cache			=	$model;

			return $cache;
		}

		if ( KunenaForum::isCompatible( '7.0' ) ) {
			KunenaForum::loadApi();
		} elseif ( checkJversion( '6.0+' ) ) {
			// Kunena 6 (Kunena 7+ this is handled by loadApi) has some critical APIs inside of the component (e.g. constants) instead of in the library so boot the component
			Application::Cms()->getApplication()->bootComponent( 'com_kunena' );
		}
