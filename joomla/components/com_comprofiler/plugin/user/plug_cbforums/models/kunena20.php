...
class cbforumsModel extends cbPluginHandler
{
	...
	static public function getPosts( $viewer, $user, $tab, $plugin )
	{
		...
		if ( ! cbforumsClass::getModel()->version ) {
	...
	static public function unFavorite( $postid, $user, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		...
		if ( $postid == 'all' ) {
			$topics	=	KunenaTopicHelper::getLatestTopics( false, 0, 0, [ 'user' => (int) $user->id, 'favorited' => true ] );
	...
	static public function unSubscribe( $postid, $user, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		...
		if ( $postid == 'all' ) {
			$topics	=	KunenaTopicHelper::getLatestTopics( false, 0, 0, [ 'user' => (int) $user->id, 'subscribed' => true ] );
			$ids	=	array_keys( array_pop( $topics ) );
		} else {
			$ids	=	array( (int) $postid );
		}
			...
			foreach ( $fields as $field ) {
				...
				if ( $cbField && isset( $user->$cbField ) ) {
					...
					if ( $field == 'website' ) {
						...
						if ( $webName != $exists->websitename ) {
							$exists->websitename	=	$webName;

							$updated	=	true;
						}

						if ( $webUrl != $exists->websiteurl ) {
							$exists->websiteurl		=	$webUrl;

							$updated	=	true;
						}
					} elseif ( isset( $exists->$field ) ) {
						if ( $value != $exists->$field ) {
							$exists->$field	=	$value;
