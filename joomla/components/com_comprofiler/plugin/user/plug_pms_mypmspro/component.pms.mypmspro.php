...
class CBplug_pmsmypmspro extends cbPluginHandler
{
	...
	public function showMessages( $user, $type = null )
	{
		...
		require PMSHelper::getTemplate( null, 'messages' . ( $type === 'modal' ? '_modal' : '' ) );
	}
	...
	public function showMessage( $id, $user )
	{
		...
		if ( $messageEditor >= 2 ) {
			$input['message']			=	Application::Cms()->displayCmsEditor( 'message', $this->getInput()->getHtml( 'post/message', '' ), '100%', 350, 70, 6 );
		} else {
			$input['message']			=	'<textarea id="message" name="message" class="w-100 form-control" cols="35" rows="6"' . ( $messageLimit ? cbValidator::getRuleHtmlAttributes( 'maxlength', $messageLimit ) : null ) . '>' . htmlspecialchars( $this->getInput()->getString( 'post/message', '' ) ) . '</textarea>';
		}
	...
	public function showMessageEdit( $id, $user )
	{
		...
		if ( ( ! $user->getInt( 'id', 0 ) ) && $this->params->getInt( 'messages_public', 0 ) ) {
			$input['from_name']			=	'<input type="text" id="from_name" name="from_name" value="' . htmlspecialchars( $this->getInput()->getString( 'post/from_name', $row->getString( 'from_name', '' ) ) ) . '" class="form-control required" size="40"' . cbValidator::getRuleHtmlAttributes( 'maxlength', 100 ) . ' />';
			$input['from_email']		=	'<input type="text" id="from_email" name="from_email" value="' . htmlspecialchars( $this->getInput()->getString( 'post/from_email', $row->getString( 'from_email', '' ) ) ) . '" class="form-control required" size="40"' . cbValidator::getRuleHtmlAttributes( 'email' ) . cbValidator::getRuleHtmlAttributes( 'maxlength', 100 ) . ' />';
		}
		...
		if ( Application::MyUser()->isGlobalModerator() ) {
			if ( $this->params->getBool( 'messages_global', true ) ) {
				$input['global']		=	moscomprofilerHTML::checkboxListButtons( array( moscomprofilerHTML::makeOption( '1', '<span class="fa fa-globe"></span>', 'value', 'text', null, 'rounded-start-0' ) ), 'global', 'title="' . htmlspecialchars( CBTxt::T( 'Select if this message should be sent to all users.' ) ) . '"', 'value', 'text', $this->getInput()->getInt( 'post/global', ( $row->getInt( 'id', 0 ) && ( ! $row->getInt( 'to_user', 0 ) ) ? 1 : 0 ) ), 0, array( 'pmMessageEditGlobal' ), null, false );
			}

			if ( $this->params->getBool( 'messages_system', true ) ) {
				$input['system']		=	moscomprofilerHTML::yesnoButtonList( 'system', 'title="' . htmlspecialchars( CBTxt::T( 'Select if this message should be sent from the system. It will not link back to you personally, but the message will still belong to you.' ) ) . '"', $this->getInput()->getInt( 'post/system', $row->getInt( 'from_system', 0 ) ) );
			}
		}
		...
		if ( $listConnections ) {
			...
			$input['conn']				=	moscomprofilerHTML::selectList( $listConnections, 'connection', 'class="d-flex justify-content-center align-items-center btn btn-light border fa fa-users pmMessageEditConn" data-cbselect-selectionCssClass="hidden"', 'value', 'text', 0, 1, false, false );
		} else {
			$input['conn']				=	null;
		}

		if ( $messageEditor >= 2 ) {
			$input['message']			=	Application::Cms()->displayCmsEditor( 'message', $this->getInput()->getHtml( 'post/message', $row->getHtml( 'message', '' ) ), '100%', 350, 70, 6 );
		} else {
			$input['message']			=	'<textarea id="message" name="message" class="w-100 form-control" cols="35" rows="6"' . ( $messageLimit ? cbValidator::getRuleHtmlAttributes( 'maxlength', $messageLimit ) : null ) . '>' . htmlspecialchars( $this->getInput()->getString( 'post/message', $row->getString( 'message', '' ) ) ) . '</textarea>';
		}
