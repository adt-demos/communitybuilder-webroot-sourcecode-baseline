...
class getmypmsproTab extends cbPMSHandler
{
	 ...
	 * @return string
	 */
	public function getPMSMenuIcon( $userId )
	{
		return $this->renderIcon( $userId, 'menu' );
	}
	 ...
	 * @return string
	 */
	private function renderIcon( $userId, $in = '' )
	{
		global $_CB_framework;
		...
		if ( ! UddeIM::isUddeIM() ) {
			$loading			=	'<div class="d-flex flex-column justify-content-center align-items-center h-100 pmMessagesLoading"><div class="spinner-border"></div></div>';
			...
			$js					=	"$( '.cbPMSMessages' ).on( 'cbtooltip.render', function( e, cbtooltip ) {"
								...
								.							"api.elements.content.addClass( 'p-0' );"
								.							"if ( data ) {"
	...
	public function getDisplayTab( $tab, $user, $ui )
	{
		...
		if ( ( ! $viewer->getInt( 'id', 0 ) ) && $allowPublic ) {
			$input['from_name']			=	'<input type="text" id="from_name" name="from_name" value="' . htmlspecialchars( $this->getInput()->getString( 'post/from_name', '' ) ) . '" class="form-control required" size="40"' . cbValidator::getRuleHtmlAttributes( 'maxlength', 100 ) . ' />';
			$input['from_email']		=	'<input type="text" id="from_email" name="from_email" value="' . htmlspecialchars( $this->getInput()->getString( 'post/from_email', '' ) ) . '" class="form-control required" size="40"' . cbValidator::getRuleHtmlAttributes( 'email' ) . cbValidator::getRuleHtmlAttributes( 'maxlength', 100 ) . ' />';
		}

		if ( $messageEditor >= 2 ) {
			$input['message']			=	Application::Cms()->displayCmsEditor( 'message', $this->getInput()->getHtml( 'post/message', '' ), '100%', 350, 70, 6 );
		} else {
			$input['message']			=	'<textarea id="message" name="message" class="w-100 form-control" cols="35" rows="6"' . ( $messageLimit ? cbValidator::getRuleHtmlAttributes( 'maxlength', $messageLimit ) : '' ) . '>' . htmlspecialchars( $this->getInput()->getString( 'post/message', '' ) ) . '</textarea>';
		}
