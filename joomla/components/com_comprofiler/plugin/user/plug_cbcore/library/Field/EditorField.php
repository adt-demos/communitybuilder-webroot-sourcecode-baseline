		...
		switch ( $output ) {
			...
			case 'htmledit':
				if ( $reason == 'search' ) {
					...
					$editorWidth		=	$field->params->get( 'fieldEditorWidth', '100%', GetterInterface::STRING );
					$editorHeight		=	$field->params->get( 'fieldEditorHeight', '400px', GetterInterface::STRING );
					$loadXtdButtons		=	(bool) $field->params->get( 'fieldEditorButtons', 0, GetterInterface::INT );
					$editor				=	Application::Cms()->displayCmsEditor( $field->name, $value, $editorWidth, $editorHeight, $field->cols, $field->rows, $loadXtdButtons, moscomprofilerHTML::htmlId( $field->name ) );
