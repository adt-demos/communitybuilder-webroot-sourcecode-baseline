...
class CBEditView_html_default extends cbEditView
{
	...
	protected function renderTabs(): string
	{
		...
		$buttons		=	'<div class="row no-gutters cbProfileEditButtons">'
						.		'<div class="' . ( $this->userEditLayout === 'vertical' ? 'text-right' : 'offset-sm-3 col-sm-9' ) . '">'
...
class CBRegistrationView_html_default extends cbRegistrationView
{
	...
	protected function _renderLayout( )
	{
		...
		echo 	'<div class="cb_template cb_template_' . selectTemplate( 'dir' ) . ' cbRegistration cbRegistration' . htmlspecialchars( ucfirst( $this->userRegLayout ) ) . ( $this->titleCanvas ? ' cbRegistrationCanvas' : '' ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : '' ) . '">'
			.		$header
			.		( $this->userRegLayout !== 'custom' ? $this->integrations : '' )
...
class CBListView_html_default extends cbListView
{
	...
	protected function _renderHead( )
	{
		...
		if ( ( ! $this->hideHeader ) || $headerRightColumn ) {
			$return					.=	( $this->listTitleHtml && ( ! $this->hideHeader ) ? '<div class="m-0 border-bottom cb-page-header cbUserListTitle"><h3 class="m-0 p-0 mb-2 cb-page-header-title">' . $this->listTitleHtml . '</h3></div>' : null )
									.	'<div class="cbUserListHead">'
									.		'<div class="row no-gutters cbColumns">';

			if ( ! $this->hideHeader ) {
				$return				.=			'<div class="' . ( $headerRightColumn ? 'col-sm-9 pr-sm-3 cbColumn9' : 'col-sm-12 cbColumn12' ) . '">'
				...
				$return				.=				'</div>'
									.			'</div>';
			} elseif ( $headerRightColumn ) {
				$return				.=			'<div class="col-sm-9 pr-sm-3 cbColumn9"></div>';
			}
		...
		if ( $this->searchTabContent ) {
			$return					.=	'<div class="cbUserListSearch">'
									.		( $this->searchCriteriaTitleHtml && ( ! $this->hideHeader ) ? '<div class="mb-3 border-bottom cb-page-header cbUserListSearchTitle"><h3 class="m-0 p-0 mb-2 cb-page-header-title">' . $this->searchCriteriaTitleHtml . '</h3></div>' : null )
			...
			if ( $this->searchResultDisplaying && $this->searchResultsTitleHtml ) {
				$return				.=		( $this->searchCriteriaTitleHtml && ( ! $this->hideHeader ) ? '<div class="m-0 border-bottom cb-page-header searchCriteriaTitleHtml"><h3 class="m-0 p-0 mb-2 cb-page-header-title">' . $this->searchResultsTitleHtml . '</h3></div>' : null );
			}
	...
	protected function _renderBody( )
	{
		...
		$return							=	'<div id="cbUserTable" class="mt-0' . ( $layout == 'grid' ? ' ml-n2 mr-n2 mb-n3 row no-gutters' : null ) . ' cbUserListDiv ' . ( $layout == 'grid' ? 'cbUserListLayoutGrid' : 'cbUserListLayoutList' ) . ' cbUserListT_' . $this->listId . ( $hasCanvas ? ' cbUserListCanvas' : null ) . '" role="table">';
