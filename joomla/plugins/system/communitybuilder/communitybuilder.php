...
class plgSystemCommunityBuilder extends CMSPlugin
{
	...
	public function onContentPrepareForm( $form, $data )
	{
		if ( ( $form instanceof Form ) && ( $form->getName() == 'com_menus.item' ) ) {
			...
			if ( isset( $data['request']['option'] ) && ( $data['request']['option'] == 'com_comprofiler' ) && isset( $data['request']['view'] ) && ( $data['request']['view'] == 'pluginclass' ) ) {
				...
				if ( $element ) {
					$db			=	( Version::MAJOR_VERSION >= 6 ? Factory::getContainer()->get( DatabaseInterface::class ) : Factory::getDbo() );
	...
	public function onAfterRoute()
	{
		...
		$input								=	( Version::MAJOR_VERSION >= 6 ? $app->getInput() : $app->input );
		...
		if ( $this->isClientSite() && ( $this->params->get( 'redirect_urls', 1 ) || $this->params->get( 'verify_consent', 0 ) ) && $this->isRerouteSafe() ) {
			...
			if ( in_array( ( explode( '.', $input->get( 'task', '' ), 2 )[0] ?? '' ), $exclude, true ) || in_array( ( explode( '.', $input->get( 'view', '' ), 2 )[0] ?? '' ), $exclude, true ) ) {
				$skip						=	true;
			}
			...
			if ( ( $option == 'com_users' ) && $this->params->get( 'redirect_urls', 1 ) && ( ! $skip ) ) {
				...
				switch ( $view ) {
					...
					case 'profile.save':
					case 'profile':
						if ( ( $view == 'profile.edit' ) || ( $input->get( 'layout' ) == 'edit' ) ) {
							$view			=	'userdetails';
					...
					case 'login':
						...
						if ( $_POST && Session::checkToken() ) {
							...
							$input->set( 'option', 'com_comprofiler' );
							$input->set( 'task', 'login' );
							$input->set( 'view', 'login' );
							$input->set( $cbSpoofField, 1 );
		...
		if ( $this->isClientSite() && ( $input->get( 'option' ) == 'com_comprofiler' ) ) {
			...
			if ( $this->params->get( 'rewrite_urls', 1 ) ) {
				...
				if ( in_array( $view, [ 'saveuseredit', 'logout', 'fieldclass' ] ) || in_array( $input->get( 'format' ), [ 'raw', 'json' ], true ) ) {
				...
				// For Joomla 5.2.3+ compatibility:
				$app->set( 'site_reset_password_urls', [
					[ 'option' => 'com_comprofiler', 'view' => 'userdetails' ],
					[ 'option' => 'com_comprofiler', 'view' => 'saveuseredit' ],
					[ 'option' => 'com_comprofiler', 'view' => 'logout' ],
					[ 'option' => 'com_comprofiler', 'view' => 'fieldclass' ],
				]);
			}
		}
	}
	...
	public function onAfterInitialise()
	{
		...
		if ( $this->isClientSite() ) {
			if ( $input->get( 'option' ) == 'com_comprofiler' ) {
				// CB is dynamic and can't be page cached; so remove the cache:
				$config		=	( Version::MAJOR_VERSION >= 6 ? $app->getConfig() : Factory::getConfig() );

				if ( $config && $config->get( 'caching' ) ) {
					if ( Version::MAJOR_VERSION >= 6 ) {
						Factory::getContainer()->get( CacheControllerFactoryInterface::class )->createCacheController( 'callback', [ 'defaultgroup' => 'page' ] )->remove( Uri::getInstance()->toString() );
					} else {
	...
	private function getJoomlaUserConsented()
	{
		...
		$userId				=	(int) ( Version::MAJOR_VERSION >= 6 ? Factory::getApplication()->getIdentity()->id : Factory::getUser()->id );
	...
	private function getUserIsGuest()
	{
		...
		if ( $cache === null ) {
			$cache		=	(bool) ( Version::MAJOR_VERSION >= 6 ? Factory::getApplication()->getIdentity()->guest : Factory::getUser()->guest );
		}
	...
	private function isRerouteSafe()
	{
		...
		$config		=	( Version::MAJOR_VERSION >= 6 ? Factory::getApplication()->getConfig() : Factory::getConfig() );

		return ( $config && ( $config->get( 'offline' ) == 1 ) && $this->getUserIsGuest() ? false : true );
	}
