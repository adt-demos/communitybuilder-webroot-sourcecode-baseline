...
if ( (int) $params->get( 'maincbtpl', 1 ) || ( $mode === 11 ) ) {
	outputCbTemplate();
}

require_once( dirname( __FILE__ ) . '/helper.php' );

if ( (int) $params->get( 'cb_plugins', 1 ) || ( $mode === 11 ) ) {
	global $_PLUGINS;
...
switch( $mode ) {
	case 11: // User List
		$listId			=	(int) $params->get( 'userlist_listid', 0 );
		...
		if ( ( Application::Input()->getString( 'option', '' ) === 'com_comprofiler' )
			 && ( Application::Input()->getString( 'view', '' ) === 'userslist' )
			 && ( Application::Input()->getInt( 'listid', 0 ) === $listId )
		) {
			if ( (int) $params->get( 'userlist_hide', 1 ) ) {
				return;
			}

			$listData	=	Application::Input()->asArray();
		} else {
			$userlistMode	=	(int) $params->get( 'userlist_mode', 0 );

			if ( $userlistMode === 3 ) {
				$listParams['list_search']	=	0;
				$listData['searchmode']		=	0;
			} else {
				$listData['searchmode']		=	$userlistMode;
			}
		}

		$listParams['list_hide_header']		=	(bool) $params->get( 'userlist_header', 1 );
		$listParams['list_hide_metadata']	=	true;
		...
		ob_start();
		( new cbUsersList() )->drawUsersList( $user->getInt( 'id', 0 ), $listId, $listData, $listParams );
