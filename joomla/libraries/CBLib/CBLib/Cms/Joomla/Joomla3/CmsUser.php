...
class CmsUser implements CmsUserInterface
{
	...
	protected function __construct( $userId )
	{
		if ( $userId ) {
			if ( Version::MAJOR_VERSION >= 6 ) {
				$this->cmsOwnUser	=	Factory::getContainer()->get( UserFactoryInterface::class )->loadUserById( $userId );
			} else {
	...
	public static function getInstance( $userIdOrConditions = null )
	{
		...
		elseif ( is_array( $userIdOrConditions ) )
		{
			...
			{
				if ( Version::MAJOR_VERSION >= 6 ) {
					$jUser	=	Factory::getContainer()->get( UserFactoryInterface::class )->loadUserByUsername( $userIdOrConditions['username'] );
				} else {
	...
	public static function getIds( array $conditions, ?array $ordering = null, $offset = 0, $limit = 0 )
	{
		...
		$jUserTable	=	( Version::MAJOR_VERSION >= 6 ? new \Joomla\CMS\Table\User( $db ) : Table::getInstance( 'user' ) );
