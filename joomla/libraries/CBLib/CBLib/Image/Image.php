...
class Image {
	 ...
	 * @param array   $options              An array of image processing options to apply (e.g. animated, quality, png_compression_level, etc..)
	 */
	public function __construct( $software = 'auto', $alwaysResample = true, $maintainAspectRatio = true, array $options = [] ) {
	...
	/**
	 * Sets the array of image processing options (e.g. animated, quality, png_compression_level, etc..)
	 *
	 * @param array $options
	 */
	...
	/**
	 * Returns the array of image processing options (e.g. animated, quality, png_compression_level, etc..)
	 *
	 * @return array
	 */
	public function getOptions(): array
	{
		...
		if ( ( ! isset( $options['animated'] ) )
			 && $this->getSource()
			 && isset( $this->getSource()['name'] )
			 && ( $this->getCleanExt() === 'gif' )
		) {
		...
		if ( ( ! isset( $options['quality'] ) )
			 ...
			 && ( ! isset( $options['webp_quality'] ) )
			 && ( ! isset( $options['avif_quality'] ) )
		) {
			$quality					=	Application::Config()->getString( 'imageQuality', '' );
	...
	public function processImage( $maxWidth, $maxHeight, $maintainAspectRatio = null, $alwaysResample = null ) {
		...
		if ( ( $imageWidth > $maxWidth ) || ( $imageHeight > $maxHeight ) || $alwaysResample ) {
			...
			if ( ! $maintainAspectRatio ) {
				$image						=	$image->resize( $newImageSize )->save( $destination . $filename, $options );
			} else {
				...
				$image						=	$image->thumbnail( $newImageSize, $imageMode )->save( $destination . $filename, $options );
			}
		} else {
			$image							=	$image->save( $destination . $filename, $options );
		}
	...
	public function resizeImage( $width, $height, $maintainAspectRatio = null, $save = false ) {
		...
		if ( $save ) {
			$image						=	$image->save( $destination . $filename, $this->getOptions() );
		}
