	...
	protected function _renderLine( $param, $result, $control_name='params', $htmlFormatting = 'div', $htmlid = true, /** @noinspection PhpUnusedParameterInspection */ $view = false ) {
		...
		if ( $htmlFormatting == 'table' ) {
			...
			if ( trim( $result[0] ) === '' && $twoLine ) {
				$html[]		=		'<td' . ( $htid ? str_replace( 'cbfr_', 'cbfv_', $htid ) : null ) . ' class="fieldCell' . ( $fieldClasses !== '' ? ' ' . htmlspecialchars( $fieldClasses ) : '' ) . '" colspan="' . ( $result[2] ? 2 : 3 ) . '" style="width: ' . ( $result[2] ? 95 : 100 ) . '%;">'
							.			$result[1]
							.		'</td>';
			} else {
				...
				$html[]		=		'<td' . ( $htid ? str_replace( 'cbfr_', 'cbfv_', $htid ) : null ) . ' class="fieldCell' . ( $fieldClasses !== '' ? ' ' . htmlspecialchars( $fieldClasses ) : '' ) . '"' . ( $twoLine ? ( ! $result[2] ? ' colspan="3"' : ' colspan="2"' ) : ( ! $result[2] ? ' colspan="2"' : null ) ) . ' style="width: ' . ( $twoLine ? ( $result[2] ? 95 : 100 ) : ( $result[2] ? 70 : 75 ) ) . '%;">'
							.			$result[1]
							.		'</td>';
			}
			...
			$html[]			=		'<div class="cb_field ' . $divSpan . '">'
							.			'<div' . ( $htid ? str_replace( 'cbfr_', 'cbfv_', $htid ) : null ) . ( ( ! $tag ) && ( ! $twoLine ) ? ' class="form-control-plaintext' . ( $fieldClasses !== '' ? ' ' . htmlspecialchars( $fieldClasses ) : '' ) . '"' : ( $fieldClasses !== '' ? ' class="' . htmlspecialchars( $fieldClasses ) . '"' : '' ) ) . '>'
							.				$result[1]
			...
			if ( substr( $result[0], -2 ) == "%s" ) {
				...
				$html[]		=		'<span class="cbFieldSpan' . ( $fieldClasses !== '' ? ' ' . htmlspecialchars( $fieldClasses ) : '' ) . '">'
							.			$result[1]
							.		'</span>'
				...
				$html[]		=	'<span' . $htid . ' class="cbft_' . $type . ( $tag ? ' cbtt_' . $tag : null ) . ( $classes !== '' ? ' ' . htmlspecialchars( $classes ) : '' ) . ( $conditioned ? ' hidden cbDisplayDisabled' : null ) . '">'
							.		'<span class="cbFieldSpan' . ( $fieldClasses !== '' ? ' ' . htmlspecialchars( $fieldClasses ) : '' ) . '">'
							.			$result[1]
							.		'</span>';
	...
	*/
			...
			case 'if':
				...
				if ( ( $ifType === 'showhide' ) && ( ! ( $viewType === 'view' ) ) ) {
					...
					if ( strpos( $param->attributes( 'name' ), 'parent/' ) === 0 ) {
						...
						// If nested inside of a repeat usage also removes the repeat row index to allow breaking out into the parent safely:
						// Additionally allow going back as many parents as necessary with multiple parent/ (e.g. parent/parent/name):
						$parentPattern				=	( $this->_isrepeat ? '/\[[^][]+](?:\[\d+])?$/' : '/\[[^][]+]$/' );
						$parentCount				=	substr_count( $param->attributes( 'name' ), 'parent/' );

						if ( $parentCount > 1 ) {
							for ( $i = 0; $i < $parentCount; $i++ ) {
								$if_control_name	=	preg_replace( $parentPattern, '', $if_control_name );
							}
						} else {
							$if_control_name		=	preg_replace( $parentPattern, '', $if_control_name );
						}
		...
		$viewName	=	preg_replace_callback( '/{(?:([^:]+):)?([^}]+)}/', function( array $matches ) use ( $subFolders, $that )
						{
							if ( $matches[1] != 'raw' ) {
								if ( $subFolders ) {
									$replace			=	str_replace( '.', '/', (string) $that->get( $matches[2], '' ) );
								} else {
									$replace			=	preg_replace( '/^([^.]+\.|\/)+/', '', (string) $that->get( $matches[2], '' ) );
								}
							} else {
								$replace				=	(string) $that->get( $matches[2], '' );
							}
	...
	public static function buildClasses( &$node, $classes = array(), $element = 'inline-block' ) {
		...
		if ( $alertClass && in_array( $alertClass, array( 'primary', 'success', 'info', 'warning', 'danger', 'light', 'dark', 'default', 'muted', 'inverse' ) ) ) {
			...
			} elseif ( $element === 'block' ) {
				if ( ! \in_array( 'm-0', $classes, true ) ) {
					$classes[]				=	'm-0';
				}
			}
			...
			if ( ( $node->attributes( 'type' ) == 'field' ) && ( $node->attributes( 'key' ) == 'tablecolumns' ) ) {
				foreach ( $sqlOptions as $k => $sqlOption ) {
					...
					if ( count( $tableColumns ) <= 1 ) {
						if ( \in_array( $node->attributes( 'title' ), [ 'title', 'description' ], true ) ) {
							$sqlOption->text		=	CBTxt::T( $sqlOption->text ) . ' (' . $sqlOption->value . ')';
							$sqlOption->translate	=	false;
						}
	...
	protected function _form_htmlarea( $name, $value, $node, $control_name ) {
		...
		if ( $this->_view ) {
			...
			$content			=	'<div id="cbdiv_' . $editorDivId . '" class="clearfix">'
				.	str_replace( '<'.'textarea', '<textarea' . $this->_title( $node ), Application::Cms()->displayCmsEditor( $editorFieldName, $value, $width, $height, $cols, $rows, true, $editorDivId ) )
				.	"</div>\n"
			;
	 ...
	 * @return string                             The html for the element
	 */
	protected function _form_icon( $name, $value, $node, $control_name ) {
		$cssclass	=	RegistryEditView::buildClasses( $node, array(), 'inline' );
		$id			=	$this->control_id( $control_name, $name );

		return '<span' . ( $cssclass ? ' class="' . \htmlspecialchars( $cssclass ) . '"' : '' ) . ( $id ? ' id="' . \htmlspecialchars( $id ) . '"' : '' ) . '></span>';
	}
	...
	protected function _form_httprequest( /** @noinspection PhpUnusedParameterInspection */ $name, $value, $node, $control_name ) {
		...
		try {
			$result				=	$client->get( $url, [ 'timeout' => 10, 'connect_timeout' => 3 ] );
	...
	protected static function savePerms( $name, $data, $title ) {
		if ( isset( $data['rules'] ) ) {
			...
			$asset				=	( checkJversion( '6.0+' ) ? new Asset( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'asset' ) );

			/** @var Asset $asset */
			if ( ! $asset->loadByName( $name ) ) {
				/** @var Asset $root */
				$root			=	( checkJversion( '6.0+' ) ? new Asset( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'asset' ) );
		...
		if ( count( $names_values ) > 0 ) {
			...
			foreach ( $names_values as $k => $v ) {
				...
				if ( is_array( $v ) ) {
					...
					/** @var $v string */
					$jsArray[$k]		.=	$v;
				} elseif ( \is_string( $v ) && $v && ( ( $v[0] === '{' ) || ( $v[0] === '[' ) ) && json_decode( $v, true, 512, JSON_THROW_ON_ERROR ) ) {
					/** @var $v string */
					$jsArray[$k]		.=	$v;
				} else {
