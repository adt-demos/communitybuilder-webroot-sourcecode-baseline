...
class Menu {
	...
	protected function renderMenu( &$modelView, &$modelRows, &$controllerView, $options, /** @noinspection PhpUnusedParameterInspection */ $htmlFormatting  ) {
		...
		$iconclass				=	$modelView->attributes( 'iconclass' );

		if ( $iconclass ) {
			// empty the icon class as we don't want to handle it in cssclass as that would append it to the menu container
			$modelView->addAttribute( 'iconclass', '' );

			// build a new node passing it the icon details
			$iconClassNode		=	new SimpleXMLElement( '<icon></icon>' );

			$iconClassNode->addAttribute( 'iconclass', $iconclass );
			$iconClassNode->addAttribute( 'iconsize', $modelView->attributes( 'iconsize' ) );
			$iconClassNode->addAttribute( 'iconstyle', $modelView->attributes( 'iconstyle' ) );

			// now parse it for the icon css classes that we'll insert as an inline span
			$iconclass			=	RegistryEditView::buildClasses( $iconClassNode, [], 'block' );
		}
		...
		if ( $label ) {
			$header				=	'<div class="text-large cbButtonMenuItemLabel"><span>' . CBTxt::Th( $label ) . '</span></div>';
		} else {
			$header				=	null;
		}
		...
		$footer					=	( $modelHtml ? '<div class="flex-grow-1 align-content-center cbButtonMenuItemData text-small text-muted">' . $modelHtml . '</div>' : null );
		...
		$return					.=			'<div class="d-flex flex-column gap-2 w-100 h-100 cbButtonMenuItemContent' . ( $name ? ' cbDIIM' . htmlspecialchars( $name ) : null ) . ( ( ! $icon ) && ( ! $iconclass ) ? ' cbButtonMenuItemContentNoIcon' : null ) . '">'
								.				$header
								...
								.				( $iconclass ? '<span class="d-block flex-grow-1 align-content-center ' . htmlspecialchars( $iconclass ) . '"' . ( $iconattributes ? ' ' . $iconattributes : '' ) . '></span>' : null )
								.				$footer
								.			'</div>';
