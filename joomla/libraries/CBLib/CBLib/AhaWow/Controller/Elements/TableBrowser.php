							...
							try {
								...
								foreach ( $exportData as $rowData ) {
									...
									foreach ( $rowData as $dataName => $dataValue ) {
										if ( ! \in_array( $dataName, $csvHeaders, true ) ) {
											$csvHeaders[$dataName]	=	$dataName;
										}
									}
								}

								fputcsv( $exportFile, $csvHeaders, ",", '"', '' );
								...
								foreach ( $exportData as $rowData ) {
									...
									foreach ( $csvHeaders as $csvHeader ) {
										...
										if ( is_string( $dataValue ) && ( $dataValue !== '' ) && ( ! is_numeric( $dataValue ) ) && in_array( $dataValue[0], array( '=', '+', '-', '@' ), true ) ) {
											// Prefix with a space to neutralise CSV/formula injection (CWE-1236):
											$dataValue				=	' ' . $dataValue;
										}

										$csvRow[$csvHeader]			=	$dataValue;
									}
				...
				if ( $this->import && Access::authorised( $this->import ) ) {
					...
					foreach ( $importData as $rowId => $rowData ) {
						...
						if ( is_array( $rowKeys ) ) {
							...
							if ( $row instanceof UserTable ) {
								// we're a user import and we already know what to import by
								if ( \array_key_exists( 'id', $rowData ) ) {
									$loadBy['id']		=	$rowData['id'];
								}

								if ( \array_key_exists( 'username', $rowData ) ) {
									$loadBy['username']	=	$rowData['username'];
								}

								if ( \array_key_exists( 'email', $rowData ) ) {
									$loadBy['email']	=	$rowData['email'];
								}
							} else {
							...
							if ( $loadBy ) {
								if ( $override != 3 ) {
									...
									if ( $row instanceof UserTable ) {
										// user table import lets try to load by user id, then username, and finally email address
										if ( \array_key_exists( 'id', $loadBy ) ) {
											$row->load( (int) $loadBy['id'] );

											$rowExists	=	$row->getInt( 'id', 0 );
										}

										if ( ( ! $rowExists ) && \array_key_exists( 'username', $loadBy ) ) {
											$row->loadByUsername( $loadBy['username'] );

											$rowExists	=	$row->getInt( 'id', 0 );
										}

										if ( ( ! $rowExists ) && \array_key_exists( 'email', $loadBy ) ) {
											$row->loadByEmail( $loadBy['email'] );

											$rowExists	=	$row->getInt( 'id', 0 );
										}
									} elseif ( \count( $loadBy ) === 1 && \array_key_exists( 'id', $loadBy ) ) {
										// we're sure we have an id primary key so lets be a bit more strict about it
										$row->load( (int) $loadBy['id'] );

										$rowExists	=	$row->getInt( $row->getKeyName(), 0 );
									} else {
										$row->load( $loadBy );
						...
						// Now lets get the table structure and expected data types so we can safely cast our data to the correct types
						if ( $tableTypes === null ) {
							global $_CB_database;

							$tableTypes				=	( $_CB_database->getTableFields( $row->getTableName() )[$row->getTableName()] ?? [] );
						}
						...
						foreach ( $rowData as $dataKey => $dataValue ) {
							...
							switch ( $tableTypes[$dataKey] ?? '' ) {
								...
								case 'float unsigned':
									...
									break;
								case 'datetime':
									if ( $dataValue && ( $dataValue !== '0000-00-00 00:00:00' ) ) {
										$dataValue	=	Application::Date( $dataValue, 'UTC' )->format( 'Y-m-d H:i:s' );
									}
									break;
								case 'date':
									if ( $dataValue && ( $dataValue !== '0000-00-00' ) ) {
										$dataValue	=	Application::Date( $dataValue, 'UTC' )->format( 'Y-m-d' );
									}
									break;
								case 'time':
									if ( $dataValue && ( $dataValue !== '00:00:00' ) ) {
										$dataValue	=	Application::Date( $dataValue, 'UTC' )->format( 'H:i:s' );
									}
									break;
							}
	...
	protected function renderList( &$modelView, &$modelRows, &$controllerView, &$options, $viewType = 'view'  ) {
		...
		if ( $tableMenu || $controllerView->hasSearchFields() || $controllerView->hasOrderbyFields() || ( count( $filtersArray ) > 0 ) || ( count( $batchArray ) > 0 ) || $exportButton || $importUpload ) {
			...
			if ( $controllerView->hasSearchFields() || $controllerView->hasOrderbyFields() || ( count( $filtersArray ) > 0 ) || ( count( $batchArray ) > 0 ) || $exportButton || $importUpload ) {
				...
				if ( count( $filtersArray ) > 0 ) {
					$return					.=			'<div class="col-12 mt-2 cbFilters cbSearchTools hidden">'
				...
				if ( count( $batchArray ) > 0 ) {
					$return					.=			'<div class="col-12 mt-2 cbBatchProcess cbBatchTools hidden">'
											.				'<fieldset class="border d-block p-2 w-100 cbFieldset">'
				...
				if ( $importUpload ) {
					$return					.=			'<div class="col-12 mt-2 cbImport cbImportTools hidden">'
											.				'<fieldset class="border d-block p-2 w-100 cbFieldset">'
