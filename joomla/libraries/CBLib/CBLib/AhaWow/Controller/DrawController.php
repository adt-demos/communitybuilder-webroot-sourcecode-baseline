...
class DrawController {
	...
	public function quicksearchfields() {
		...
		if ( $this->hasSearchFields() ) {
			...
			$return					.=	( $quickSearchLabel ? '<label for="' . $this->fieldId( 'search' ) . '">' . htmlspecialchars( $quickSearchLabel ) . ':</label> ' : null )
									...
									.			'<button class="btn btn-light border" type="button" onclick="' . htmlspecialchars( $quickSearchJS ) . '"><span class="fa fa-search"></span></button>'
									.		'</div>'
									.	'</div>';
		}
	...
	public function import( $editRowView )
	{
		...
		$input				=	'<div class="w-100 h-100 p-0 m-0 form-control text-center cbImportArea">'
