...
//EXIT;
	...
	public function process_filter( $filter, $filterValuesArray, $valueType )
	{
		...
		if ( ! is_array( $filterValuesArray['valuefield'] ) ) {
			...
			// ensure IN queries with int/string values are actually arrays and if they aren't put them in one
			if ( \in_array( $filterValuesArray['operator'], [ 'IN', 'NOT IN' ], true )
				 && ( ! \is_array( $filterValuesArray['internalvalue'] ) )
				 && \in_array( $valueType, [ 'sql:int', 'sql:string' ], true )
			) {
				$filterValuesArray['internalvalue']	=	\explode( '|*|', $filterValuesArray['internalvalue'] );
			}
