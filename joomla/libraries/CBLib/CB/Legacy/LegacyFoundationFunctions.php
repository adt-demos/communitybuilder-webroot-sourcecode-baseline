...
//	use JVersion;
	 ...
	 *
	 * @param int|bool    $htmlspecialchars TRUE if should return "&amp:Itemid...." instead of "&Itemid..." (with FALSE as default), === 0 if return only int
	 ...
	 * @param bool        $fallback         TRUE: fallback to profile menu item if specific menu item can't be found, FALSE: just omit the menu item entirely if nothing specific found
	 * @param bool        $matchExactly     TRUE: this will attempt to match the menu item URL exactly instead of with % wildcard at the end, FALSE: this will wildcard match the menu item url with % at the end
	 * @return null|string|int
	 */
	function getCBprofileItemid( $htmlspecialchars = false, $view = 'userprofile', $additional = null, bool $fallback = true, bool $matchExactly = false ) {
		global $_CB_database, $_CB_framework;

		if ( ! $view ) {
			if ( $fallback ) {
				return getCBprofileItemid( $htmlspecialchars );
			}
		...
		$cacheId				=	$view . ( $additional ? ':' . $additional : '' ) . ( $matchExactly ? ':exact' : '' );
		...
		if ( ! \array_key_exists( $cacheId, $cache ) ) {
			...
			if ( ! $Itemid ) {
				...
				if ( checkJversion( '<5.0' ) ) {
					// if older than Joomla 5 we still need to check for &task legacy menu items otherwise we can skip checking &task for faster lookup peformance
					if ( $view === 'userprofile' ) {
						$query	.=	" WHERE ( " . $_CB_database->NameQuote( 'link' ) . " = " . $_CB_database->Quote( 'index.php?option=com_comprofiler&view=userprofile' )
						...
						$query	.=	" WHERE ( " . $_CB_database->NameQuote( 'link' ) . " = " . $_CB_database->Quote( 'index.php?option=com_comprofiler&view=' . $view . $additional )
								.	" OR " . $_CB_database->NameQuote( 'link' ) . " = " . $_CB_database->Quote( 'index.php?option=com_comprofiler&task=' . $view . $additional ) . " )";
					} else {
						$query	.=	" WHERE ( " . $_CB_database->NameQuote( 'link' ) . " LIKE " . $_CB_database->Quote( 'index.php?option=com_comprofiler&view=' . $_CB_database->getEscaped( $view . $additional, true ) . '%', false )
								.	" OR " . $_CB_database->NameQuote( 'link' ) . " LIKE " . $_CB_database->Quote( 'index.php?option=com_comprofiler&task=' . $_CB_database->getEscaped( $view . $additional, true ) . '%', false ) . " )";
					}
				...
				$query			.=	" AND " . $_CB_database->NameQuote( 'published' ) . " = 1"
