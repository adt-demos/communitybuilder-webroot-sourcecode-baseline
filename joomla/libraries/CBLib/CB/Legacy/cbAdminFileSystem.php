...
use Joomla\CMS\Filesystem\File as LegacyFile;
use Joomla\Filesystem\File;
use Joomla\CMS\Filesystem\Folder as LegacyFolder;
use Joomla\Filesystem\Folder;
use Joomla\CMS\Filesystem\Path as LegacyPath;
...
if (is_callable("jimport")) {
	...
	$CB_AdminFileFunctions	=	array(	'_constructor'		=>	function ( &$functions ) {
			...
			jimport('joomla.filesystem.path');
											 ...
											 'mkdir'				=>	array( ( checkJversion( '6.0+' ) ? Folder::class : LegacyFolder::class ), 'create' ),
											 'rmdir'				=>	array( ( checkJversion( '6.0+' ) ? Folder::class : LegacyFolder::class ), 'delete' ),
											 'is_dir'			=>	null,
											 'opendir'			=>	null,
											 'readdir'			=>	null,
											 'closedir'			=>	null,
											 'rename'			=>	function ( $old_name, $new_name ) {
													 if ( is_file( $old_name ) ) {
														 if ( checkJversion( '6.0+' ) ) {
															 return File::move( $old_name, $new_name );
														 }
														 return LegacyFile::move( $old_name, $new_name );
													 } elseif ( is_dir( $old_name ) ) {
														 if ( checkJversion( '6.0+' ) ) {
															 return Folder::move( $old_name, $new_name );
														 }
														 return LegacyFolder::move( $old_name, $new_name );
													 } else {
														 return false;
													 }
												 },
											 ...
											 'chmod'				=>	function ( $pathname, $mode ) {
													 ...
													 if ( $FTPOptions['enabled'] == 1 ) {
														 ...
														 $dest		=	( checkJversion( '6.0+' ) ? Path::class : LegacyPath::class )::clean(str_replace( JPATH_ROOT, $FTPOptions['root'], $pathname), '/' );
											 ...
											 'chmoddir'			=>	null,
											 'copy'				=>	array( ( checkJversion( '6.0+' ) ? File::class : LegacyFile::class ), 'copy' ),
											 'copydir'			=>	array( ( checkJversion( '6.0+' ) ? Folder::class : LegacyFolder::class ), 'copy' ),
											 'unlink'			=>	array( ( checkJversion( '6.0+' ) ? File::class : LegacyFile::class ), 'delete' ),
											 'file_put_contents'	=>	array( ( checkJversion( '6.0+' ) ? File::class : LegacyFile::class ), 'write' ),
											 'move_uploaded_file'=>	array( ( checkJversion( '6.0+' ) ? File::class : LegacyFile::class ), 'upload' ),
	);
}
