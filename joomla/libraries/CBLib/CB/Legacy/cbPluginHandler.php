...
class cbPluginHandler
{
	...
	public function getPluginVersion( $plugin, $raw = false, $duration = null, $length = 0 )
	{
			...
			if ( Application::Config()->get( 'pluginVersionCheck', true, GetterInterface::BOOLEAN ) ) {
				...
				if ( $request && ( ! $plgVersionsRequested ) ) {
					...
					try {
						...
						$results						=	$guzzleHttpClient->get( 'https://update.joomlapolis.net/' . ( Application::Config()->getString( 'updateChannel', '' ) === 'stable' ? 'cbpluginsstableversions20.xml' : 'cbpluginsversions20.xml' ), array( 'timeout' => 10, 'connect_timeout' => 3 ) );
		...
		if ( $latestCoreVersion === null ) {
			...
			if ( $request && ( ! $coreVersionRequested ) ) {
				...
				try {
					$guzzleHttpClient					=	new GuzzleHttp\Client();
					$guzzleRequest						=	$guzzleHttpClient->get( 'https://www.joomlapolis.com/versions/comprofilerversion.php?currentversion=' . urlencode( CBLib::versionWithBuild() ), array( 'headers' => array( 'referer' =>  $_CB_framework->getCfg( 'live_site' ) ), 'timeout' => 10, 'connect_timeout' => 3 ) );
		...
		if ( ! isset( $cache[$pluginId][$raw] ) ) {
			if ( $plugin->iscore ) {
				...
				if ( $plugin->manifest_cache ) {
					$version							=	( new Registry( $plugin->manifest_cache ) )->getString( 'release', '' );
				}
	...
	public function checkPluginCompatibility( $plugin )
	{
		...
		if ( $plugin->iscore ) {
			// Core plugins are always compatible
			return true;
		}
		...
		if ( $cbVersion === null ) {
			...
			if ( preg_match( '/^(\d{2,})(\.[a-zA-Z].+)/i', $cbVersion, $matches ) ) {
		...
		if ( ! isset( $cache[$pluginId] ) ) {
			...
			if ( $plugin->manifest_cache ) {
				$version					=	( new Registry( $plugin->manifest_cache ) )->getString( 'version', '' );
			}
			...
			if ( $version ) {
				$version					=	preg_replace( '/\.*([a-zA-Z]+)\.*/i', '.$1.', preg_replace( '/^[a-zA-Z]+/i', '', str_replace( [ '-', '_', '+' ], '.', str_replace( ' ', '', strtoupper( $version ) ) ) ) );

				if ( preg_match( '/^(\d{2,})(\.[a-zA-Z].+)/i', $version, $matches ) ) {
					$version				=	implode( '.', str_split( $matches[1] ) ) . $matches[2];
				}

				$version					=	trim( str_replace( '..', '.', $version ), '.' );

				$compatible					=	( version_compare( $version, ( $cbVersion[0] . '.0' ) ) >= 0 );
			} else {
				$compatible					=	false;
			}
