...
class CBframework
{
	...
	public function viewUrl( $task, $htmlspecialchars = true, $variables = array(), $format = 'html', $ssl = 0, $raw = false )
	{
		...
		$Itemid						=	( $urlVariables['Itemid'] ?? null );

		if ( $Itemid === 0 ) {
			unset( $urlVariables['Itemid'] );
		}

		return $this->cbSef( 'index.php?option=com_comprofiler' . $extra . ( $Itemid === null ? getCBprofileItemid( false, $urlVariables['view'], $additional ) : '' ), $htmlspecialchars, $format, $ssl, ( ! $raw ) );
	}
