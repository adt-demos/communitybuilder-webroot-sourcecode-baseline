...
class CBInstallPlugin extends Table
{
	...
	public function displayWeb()
	{
		...
		$cms											=	( checkJversion( '6.0+' ) ? 'j60' : ( checkJversion( '5.0+' ) ? 'j50' : ( checkJversion( '4.0+' ) ? 'j40' : 'j30' ) ) );

		if ( $webJSON ) {
			// Ensure the POST was sent from install from web and ensure Super User permission before storing the JSON; DO NOT REMOVE:
			if ( Application::Session()->checkFormToken( 'post', 0 ) && Application::MyUser()->isSuperAdmin() ) {
				if ( $webJSON === 'request' ) {
					...
					try {
						...
						if ( cbGuzzleVersion() >= 6 ) {
							...
							$result						=	$client->get( 'https://www.joomlapolis.com/index.php', [ 'timeout' => 10, 'connect_timeout' => 3, 'query' => $query ] );
						} else {
							$result						=	$client->get( 'https://www.joomlapolis.com/index.php?option=com_comprofiler&view=pluginclass&plugin=cbpackagebuilder&action=web&format=raw', [ 'timeout' => 10, 'connect_timeout' => 3, 'query' => $query ] );
						}
		...
		if ( Application::Input()->getString( 'format', '' ) !== 'raw' ) {
			...
			$js											=	"let searchable = [];"
														...
														.		"});"
														.	"};"
														.	"$( '.cbWeb' ).on( 'click', '.cbWebType > .cbWebTypeLink', function( e ) {"
														.		"e.preventDefault();"
														...
														.		"e.preventDefault();"
														.		"$.cbweb( true );"
														.	"}).on( 'click', '.cbWebSearchCancel', function() {"
														.		"$( '.cbWebSearch' ).val( '' ).trigger( 'change' ).focus();"
														.	"}).on( 'change keyup', '.cbWebSearch', function() {"
		...
		if ( $packagesJSON ) {
			if ( $packagesJSON->has( 'packages.' . $cms ) ) {
				$packages								=	$packagesJSON->subTree( 'packages.' . $cms );
			} else {
				$packages								=	$packagesJSON->subTree( 'packages' );
			}
		} else {
			$packages									=	false;
		}
		...
		if ( $updates ) {
			$updateAll									=	'https://www.joomlapolis.com/index.php?option=com_comprofiler&view=pluginclass&plugin=cbpackagebuilder&action=packages&func=download&packages=' . implode( ',', cbToArrayOfInt( $updates ) ) . ( $accessToken ? '&token=' . urlencode( $accessToken ) : null ) . ( Application::Config()->getString( 'updateChannel', '' ) === 'stable' ? '&channel=stable' : '' ) . '&cms=' . urlencode( $cms ) . '&format=raw';
		}
		...
		$return											.=	'<div class="d-flex flex-column flex-lg-row gap-4 cbWebResponse">'
														...
														.				'</div>'
														.			'</div>'
														.			'<ul class="cbWebTypes nav flex-column nav-pills bg-body-tertiary">';
		...
		foreach ( $items as $type => $typeItems ) {
			...
			foreach ( $typeItems as $itemId => $item ) {
				...
				if ( $buttons ) {
					$return								.=							'<div class="bg-transparent card-footer p-1 cbWebPackageActions">'
														.								$buttons
														.							'</div>';
				}
				...
				try {
					...
					if ( $fromWeb ) {
						...
						if ( cbGuzzleVersion() >= 6 ) {
							...
							$guzzleRequest		=	$guzzleHttpClient->get( 'https://www.joomlapolis.com/index.php', array( 'timeout' => 90, 'connect_timeout' => 3, 'query' => array_merge( $urlQuery, $query ) ) );
						} else {
							...
							$guzzleRequest		=	$guzzleHttpClient->get( $userfileURL, array( 'timeout' => 90, 'connect_timeout' => 3, 'query' => $query ) );
						}
					} else {
