...
class cbTabs extends cbTabHandler
{
	...
	public function startPane( $id, $classes = array(), $attributes = array() )
	{
		...
		$return				=	'<div class="cbTabs' . ( $containerClass ? ' ' . htmlspecialchars( $containerClass ) : null ) . '" id="cbtabs' . htmlspecialchars( $id ) . '"' . ( Application::Config()->getInt( 'templateBootstrap4', 1 ) !== 1 ? ' data-theme="' . htmlspecialchars( Application::Cms()->getApplication()->getTemplate() ) . '"' : '' ) . ( $containerAttrs ? ' ' . $containerAttrs : null ) . '>'
							.		$navHtml
	...
	public function generateViewTabsContent( $user, $position = '', $tabId = null, $output = 'html', $formatting = null /* 'table' or 'divs' */, $reason = 'profile' )
	{
		...
		$tabId									=	( $tabId ? (int) $tabId : null );

		if ( $formatting === null ) {
			$formatting							=	'divs';
		}

		$tabOneTwoRowsStyleToggle				=	[];
		$this->action							=	1;
		...
		// If a specific tab is specified, but its position wasn't then find its position to optimize the tabsToDisplay array loop below
		if ( $tabId && ( ! $position ) ) {
			$position							=	( ( $this->tabsToDisplay[''][$tabId]->position ?? '' ) ?: 'cb_tabmain' );
		}

		// Only prepare profile menu tab menu items if we're loading all tabs, loading the menu tab, or loading the menu position
		if ( ( ( ! $tabId ) || ( $tabId === 17 ) ) && ( ( ! $position ) || ( $position === ( ( $this->tabsToDisplay[''][17]->position ?? '' ) ?: 'cb_tabmain' ) ) ) ) {
			static $menusPrepared				=	false;

			if ( ! $menusPrepared ) {
				$_PLUGINS->trigger( 'onPrepareMenus', [ &$user ] );
		...
		// Initialize tabs into their positions and call their getMenuAndStatus function to prepare profile menu and status entries
		...
		$_PLUGINS->trigger( 'onAfterPrepareViewTabs', [ &$this->tabsContents, &$this->tabsToDisplay[$position], &$user, $position, $tabId ] );
	}
	...
	protected function _callTabPlugin( &$tab, &$user, $pluginclass, $method, $pluginid = null, $postdata = null, $reason = null )
	{
		...
		if ( $pluginid ) {
			...
			if ( $_PLUGINS->loadPluginGroup( 'user', [ (int) $pluginid ] ) ) {
				$args			=	[ &$tab, &$user, $this->ui, &$postdata, $reason ];
				$results		=	implode( '', $_PLUGINS->trigger( 'onBefore' . $method, $args ) );
				...
				$_PLUGINS->trigger( 'onAfter' . $method, $args );
			}
		}
