...
// 	if(preg_match('/gallery\//i',$avatar)==false && is_file($_CB_framework->getCfg('absolute_path').'/images/comprofiler/'.$avatar)) {
	...
	function outputCbTemplate( /** @noinspection PhpUnusedParameterInspection */ $obsoleteUi = 0, $templateFile = 'template.css', $media = null ) {
		...
		if ( Application::Application()->isClient( 'administrator' ) ) {
			$loadBootstrap			=	Application::Config()->getInt( 'backendTemplateBootstrap4', 1 );
			$loadFontawesome		=	Application::Config()->getInt( 'backendTemplateFontawesme', 1 );
		} else {
			$loadBootstrap			=	Application::Config()->getInt( 'templateBootstrap4', 1 );
			$loadFontawesome		=	Application::Config()->getInt( 'templateFontawesme', 1 );
		}
		...
		if ( $templateFile === 'template.css' ) {
			...
			// Add Joomla specific fixes
			if ( checkJversion( '5.0+' ) ) {
				outputCbTemplate( null, 'template.j5.css', $media );
			} elseif ( checkJversion( '4.0+' ) ) {
				outputCbTemplate( null, 'template.j4.css', $media );
			} elseif ( checkJversion( '3.0+' ) ) {
				outputCbTemplate( null, 'template.j3.css', $media );
			}
		}
