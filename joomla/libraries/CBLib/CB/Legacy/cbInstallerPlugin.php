...
class cbInstallerPlugin extends cbInstaller
{
	...
	public function install( $fromDirectory = null, $InstallIntoDatabaseOnly = false )
	{
		...
		$THISCBVERSION			=	'2.11.0+build.2026.07.31.14.26.35.e46e38af0';               // IMPORTANT: DO NOT CHANGE MANUALLY, but when changing version automatically here also change 2 times in CBLib.php, the 8 XML installation files and build.xml
		...
		if ( $newRelease !== false ) {
			...
			$manifest->set( 'release', $newRelease );
		}
		...
		if ( $e !== false ) {
			...
			$manifest->set( 'description', $e->data() );
		}

		$row->manifest_cache	=	$manifest->asJson();
...
//$sqlUpgrader->setDryRun( true );
	...
	public function installTab( $pluginId, $tab )
	{
		...
		if ( ! $tab->attributes( 'tabid' ) ) {
			...
			$tabId			=	(int) $_CB_database->loadResult();
		} else {
			$tabId			=	(int) $tab->attributes( 'tabid' );
		}
		...
		if ( $tab->attributes( 'type' ) === 'existingSytemTab' ) {
			...
			if ( ! $row->tabid ) {
				...
				$row->ordering							=	99;
				$row->position							=	( $tab->attributes('position') ?? 'canvas_main_middle' );
				$row->displaytype						=	( $tab->attributes('displaytype') ?? 'menu' );
				$row->ordering_edit						=	(int) ( $tab->attributes('ordering_edit') ?? 1 );
				$row->ordering_register					=	(int) ( $tab->attributes('ordering_register') ?? 1 );
				$row->enabled							=	(int) ( $tab->attributes('enabled') ?? 1 );
				$row->viewaccesslevel					=	1;
			...
			$row->fields								=	(int) ( $tab->attributes('fields') ?? 1 );
			$row->sys									=	(int) ( $tab->attributes('sys') ?? 0 );
	...
	public function installField( $pluginId, $tabId, $field )
	{
		...
		if ( ! $field->attributes( 'fieldid' ) ) {
			if ( $field->attributes('name') ) {
				$_CB_database->setQuery( "SELECT fieldid FROM #__comprofiler_fields WHERE name = " . $_CB_database->Quote( $field->attributes('name') ) );
				$fieldid		=	(int) $_CB_database->loadResult();
			} else {
				$fieldid		=	0;
			}
		} else {
			$fieldid			=	(int) $field->attributes('fieldid');
		}
		...
		$row->pluginid			=	$pluginId;
		$row->type				=	( $field->attributes('type') ?? 'text' );
		$row->calculated		=	(int) ( $field->attributes('calculated') ?? 0 );
		if ( ! $row->fieldid ) {
			$row->tabid				=	$tabId;
			$row->title				=	( $field->attributes('title') ?? '' );
			$row->description		=	trim( ( $field->attributes('description') ?? '' ) );
			$row->ordering			=	99;
			$row->registration		=	(int) ( $field->attributes('registration') ?? 1 );
			$row->profile			=	(int) ( $field->attributes('profile') ?? 1 );
			$row->edit				=	(int) ( $field->attributes('edit') ?? 1 );
			$row->readonly			=	(int) ( $field->attributes('readonly') ?? 0 );
			$row->published			=	(int) ( $field->attributes('published') ?? 1 );
			$row->searchable		=	(int) ( $field->attributes('searchable') ?? 0 );
	...
	public function installFieldValue( $fieldId, $fieldValue )
	{
		...
		$row->ordering			=	(int) ( $fieldValue->attributes( 'ordering' ) ?? 1 );
		$row->sys				=	(int) ( $fieldValue->attributes( 'sys' ) ?? 0 );
