...
class cbValidator
{
	...
	/**
	 * Returns html attributes to force validation of an input on a given event
	 *
	 * @param string $on The event to force validate on (currently only supports keyup)
	 * @return string
	 */
	static function getValidateOnAttributes( $on )
	{
		return ' data-validate-on="' . htmlspecialchars( $on ) . '"';
	}
