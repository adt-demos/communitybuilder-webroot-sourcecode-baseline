...
class pkg_communitybuilderInstallerScript {
	...
	private $versions = array(
		...
		'joomla' => array (
			'7.0.0-alpha1' => '999.9',		// 999.9 = incompatible
			'6.0' => '6.0.0',
			'5.1' => '5.1.0',
			'5.0' => '5.0.0',
			'4.3' => '4.3.0',
	...
	public function install( /** @noinspection PhpUnusedParameterInspection */ $parent ) {
		$session = ( Version::MAJOR_VERSION >= 6 ? Factory::getApplication()->getSession() : Factory::getSession() );
	...
	public function postflight($type, /** @noinspection PhpUnusedParameterInspection */ $parent) {
		...
		$cache = ( Version::MAJOR_VERSION >= 6 ? Factory::getContainer()->get( CacheControllerFactoryInterface::class )->createCacheController( 'callback', [ 'defaultgroup' => '_system' ] ) : Factory::getCache() );
		$cache->clean('_system');
	...
	public function enablePlugin($group, $element) {
		$plugin = ( Version::MAJOR_VERSION >= 6 ? new Extension( Factory::getContainer()->get( DatabaseInterface::class ) ) : Table::getInstance( 'extension' ) );
