...
function checkcbdb( $dbId = 0 ) {
	...
	if ( $dbId == 0 ) {
		...
		// 15. check if plugins missing their manifest cache
		$sql = "SELECT `name`, `id` FROM `#__comprofiler_plugin` WHERE `manifest_cache` IS NULL OR `manifest_cache` = ''";
		$_CB_database->setQuery( $sql );
		$bad_rows = $_CB_database->loadObjectList();
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="mb-2 text-danger">' . sprintf( CBTxt::T( 'Warning: %s entries in Community Builder __comprofiler_plugin are missing their manifest cache.' ), count( $bad_rows ) ) . '</div>';

			foreach ( $bad_rows as $bad_row ) {
				echo '<div class="mb-2 text-danger">plugin=' . htmlspecialchars( $bad_row->name ) . ' pluginid=' . (int) $bad_row->id . '</div>';
			}

			echo '<div class="mb-2 text-danger">' . CBTxt::T( 'This one can be fixed by <strong>first backing up database</strong>' ) . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixcbmanifestdb&$cbSpoofField=$cbSpoofString" ) . '"> ' . CBTxt::T( 'then by clicking here' ) . '</a>.</div>';
		} else {
			echo '<div class="mb-2 text-success">' . CBTxt::T( 'All Community Builder __comprofiler_plugin table rows have their manifest cache.' ) . '</div>';
		}
...
function fixcbmanifestdb()
{
	global $_CB_database, $_CB_Backend_Title, $_PLUGINS;

	@set_time_limit( 240 );

	$_CB_Backend_Title			=	[ 0 => [ 'fa fa-wrench', CBTxt::T( 'CB Tools: Fix Manifest Cache: Results' ) ] ];
	$affected					=	0;

	$_CB_database->setQuery( "SELECT `id` FROM `#__comprofiler_plugin` WHERE `manifest_cache` IS NULL OR `manifest_cache` = ''" );
	foreach ( $_CB_database->loadResultArray() as $pluginId ) {
		$xmlFile				=	$_PLUGINS->getPluginXmlPath( $pluginId );
		...
		if ( file_exists( $xmlFile ) ) {
			...
			if ( $xml !== null ) {
				...
				if ( $description !== false ) {
					$manifest->set( 'description', $description->data() );
				}

				$version		=	$xml->getElementByPath( 'version' );

				if ( $version !== false ) {
					$manifest->set( 'version', $version->data() );
				}

				$release		=	$xml->getElementByPath( 'release' );

				if ( $release !== false ) {
					$manifest->set( 'release', $release->data() );
				}
			}
		}

		$_CB_database->setQuery( "UPDATE `#__comprofiler_plugin` SET `manifest_cache` = " . $_CB_database->Quote( $manifest->asJson() ) . " WHERE `id` = " . (int) $pluginId );
		try {
			$_CB_database->query();
		} catch ( RuntimeException $e ) {
			print '<div class="mb-2 text-danger">' . sprintf( CBTxt::T( 'SQL error %s' ), htmlspecialchars( $e->getMessage() ) ) . '</div>';
			continue;
		}
	...
	if ( $affected ) {
		print '<div class="mb-2 text-warning">' . sprintf( CBTxt::T( 'Fixed %s __comprofiler_plugin entries with missing manifest_cache.' ), $affected ) . '</div>';
	}

	print '<div class="mb-2 text-success">' . CBTxt::T( 'Plugin database adjustments complete! Plugins should now all have their manifest_cache!' ) . '</div>';
}
	...
	if ( file_exists( $baseDir ) ) {
		if ( $adminFS->is_writable( $baseDir ) || ! $adminFS->isUsingStandardPHP() ) {
			...
			try {
				...
				$guzzleRequest		=	$guzzleHttpClient->get( $userfileURL, array( 'timeout' => 90, 'connect_timeout' => 3 ) );
			} catch( Exception $e ) {
