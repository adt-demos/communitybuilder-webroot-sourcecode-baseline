...
class Com_ComprofilerInstallerScript {
	...
	public function install( /** @noinspection PhpUnusedParameterInspection */ $parent, $method = 'install' ) {
		...
		$return									=	'<div style="margin-bottom:10px;width:100%;text-align:center;"><img alt="' . htmlspecialchars( CBTxt::T( 'CB Logo' ) ) . '" src="' . $liveSite . '/components/com_comprofiler/images/smcblogo.gif" /></div>'
												.	'<div style="font-size:14px;margin-bottom:10px;">Copyright 2004-2025 Joomlapolis.com. ' . CBTxt::T( 'This component is released under the GNU/GPL version 2 License. All copyright statements must be kept. Derivate work must prominently duly acknowledge original work and include visible online links.' ) . '</div>';
...
function cbInstaller_install_plugins( $parent, &$return, $method ) {
	...
	// apply Joomla version based configuration to new installs only
	if ( checkJversion( '4.0+' ) ) {
		$installConfigSet							=	false;

		if ( $method === 'install' ) {
			// enable Joomla based forgot login
			$ueConfig['forgotlogin_type']			=	0;

			$installConfigSet						=	true;
		}

		if ( ! isset( $ueConfig['backendTemplateBootstrap4'] ) ) {
			// enable backend Bootstrap v5 compatibility
			$ueConfig['backendTemplateBootstrap4']	=	2;

			$installConfigSet						=	true;
		}

		if ( ( ! isset( $ueConfig['backendTemplateFontawesme'] ) ) && checkJversion( '5.0+' ) ) {
			// enable backend Fontawesome v6 compatibility
			$ueConfig['backendTemplateFontawesme']	=	2;
...
function cbInstaller_create_menutype( $type, $title, $description = null, $properties = array() ) {
	$menuType					=	( checkJversion( '6.0+' ) ? new MenuType( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'MenuType' ) );
...
function cbInstaller_create_menuitem( $menuType, $title, $url, $properties = array() ) {
	...
	$table						=	( checkJversion( '6.0+' ) ? new Menu( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'Menu' ) );
	...
	$menu						=	( checkJversion( '6.0+' ) ? new Menu( Application::Cms()->getCmsDatabaseDriver() ) : Table::getInstance( 'Menu' ) );
