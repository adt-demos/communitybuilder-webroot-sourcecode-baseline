...
class mod_cbadminInstallerScript
{
	...
	private function upgradeDefaultWidth( $moduleParams )
	{
		if ( Version::MAJOR_VERSION > 3 ) {
			...
			/** @var Module $module */
			$module							=	( Version::MAJOR_VERSION >= 6 ? new Module( Factory::getContainer()->get( DatabaseInterface::class ) ) : Table::getInstance( 'module' ) );
	...
	private function runParametersUpgrades( )
	{
		...
		$moduleParamsPlgVersion = '{"mode":"6","menu_cb":"1","menu_plugins":"1","menu_compact":"1","feed_entries":"5","feed_duration":"12","modal_display":"1","module_tag":"div","bootstrap_size":"6","header_tag":"h3","header_class":"","style":"0"}';
		...
		$this->upgradeDefaultWidth( $moduleParamsPlgVersion );
	}
	...
	public function install( $adapter )
	{
		...
		$extension						=	( Version::MAJOR_VERSION >= 6 ? new Extension( $db ) : Table::getInstance( 'extension' ) );
		...
		if ( $extension->load( array( 'element' => 'mod_cb_adminnav' ) ) ) {
			...
			if ( $modules ) {
				foreach ( $modules as $moduleId ) {
					/** @var Module $module */
					$module				=	( Version::MAJOR_VERSION >= 6 ? new Module( $db ) : Table::getInstance( 'module' ) );
		...
		if ( $module->load( array( 'module' => 'mod_cbadmin', 'position' => 'cpanel' ) ) ) {
			...
			$moduleParamsPlgVersion = '{"mode":"6","menu_cb":"1","menu_plugins":"1","menu_compact":"1","feed_entries":"5","feed_duration":"12","modal_display":"1","module_tag":"div","bootstrap_size":"12","header_tag":"h3","header_class":"","style":"0"}';
			...
			$this->adaptForJoomla3IfNeeded( $moduleParamsPlgVersion );
			...
			$module->set( 'title', 'CB Admin Plugin Version Checker' );
			$module->set( 'ordering', '99' );
			$module->set( 'position', 'cpanel' );
			$module->set( 'published', '1' );
			$module->set( 'module', 'mod_cbadmin' );
			$module->set( 'access', '1' );
			$module->set( 'showtitle', '0' );
			$module->set( 'params', $moduleParamsPlgVersion );
			$module->set( 'client_id', '1' );
			$module->set( 'language', '*' );
