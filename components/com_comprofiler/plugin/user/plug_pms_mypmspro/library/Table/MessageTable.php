...
class MessageTable extends Table
{
				...
				if ( ( $notify == 1 ) || ( ( $notify == 2 ) && ( ! $this->get( 'reply_to', 0, GetterInterface::INT ) ) ) || ( ( $notify == 3 ) && $this->get( 'reply_to', 0, GetterInterface::INT ) ) ) {
					...
					if ( $this->get( 'reply_to', 0, GetterInterface::INT ) ) {
						...
						if ( PMSHelper::getGlobalParams()->get( 'messages_notify_message', false, GetterInterface::BOOLEAN ) ) {
							$message		=	CBTxt::T( 'FROM_HAS_REPLIED_MESSAGE_WITH', '[from] has replied to your private message.<br /><br />[message]', array( '[from]' => $this->getFrom( 'profile_direct' ), '[message]' => $this->getMessage() ) );
						} else {
							$message		=	CBTxt::T( 'FROM_HAS_REPLIED_MESSAGE', '[from] has replied to your private message.', array( '[from]' => $this->getFrom( 'profile_direct' ) ) );
						}
					} else {
						$subject			=	CBTxt::T( 'You have a new private message' );

						if ( PMSHelper::getGlobalParams()->get( 'messages_notify_message', false, GetterInterface::BOOLEAN ) ) {
							$message		=	CBTxt::T( 'FROM_HAS_SENT_MESSAGE_WITH', '[from] has sent you a new private message.<br /><br />[message]', array( '[from]' => $this->getFrom( 'profile_direct' ), '[message]' => $this->getMessage() ) );
						} else {
							$message		=	CBTxt::T( 'FROM_HAS_SENT_MESSAGE', '[from] has sent you a new private message.', array( '[from]' => $this->getFrom( 'profile_direct' ) ) );
						}
					}
	...
	public function getRead( $userId = 0 )
	{
		...
		$readDate					=	$this->get( '_read', null, GetterInterface::STRING );
