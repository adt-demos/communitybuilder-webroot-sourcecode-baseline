...
class CBplug_pmsmypmspro extends cbPluginHandler
{
		...
		if ( ! $row->get( 'id', 0, GetterInterface::INT ) ) {
			...
			foreach ( $toArray as $k => $to ) {
				...
				if ( $reply && ( ! $replyTo->get( 'from_user', 0, GetterInterface::INT ) ) ) {
					...
					$message			=	CBTxt::T( 'FROM_HAS_REPLIED_MESSAGE', '[from] has replied to your private message.<br /><br />[message]', array( '[from]' => $row->getFrom( 'profile_direct' ), '[message]' => $row->getMessage() ) );
	...
	private function reportMessage( $id, $user )
	{
		...
		PMSHelper::returnRedirect( $returnUrl, CBTxt::T( 'MESSAGE_REPORTED_SUCCESS', 'Message reported successfully! <a href="[delete_url]">Click here if you would also like to delete this message.</a>', array( '[delete_url]' => $_CB_framework->pluginClassUrl( $this->element, true, array( 'action' => 'message', 'func' => 'delete', 'id' => $row->getInt( 'id', 0 ), cbSpoofField() => cbSpoofString( null, 'plugin' ) ) ) ) ) );
	}
}